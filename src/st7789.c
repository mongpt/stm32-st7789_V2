...
static void ST7789_WriteData(uint8_t *buff, size_t buff_size)
{
    ...
    while (buff_size > 0) {
        ...
        if ((ST7789_DMA_MIN_TX_BYTES <= chunk_size) && (ST7789_SPI_PORT.hdmatx != NULL)) {
             ...
             * and skip bytes — common symptom is a blank LCD with UART still fine. */
...
void ST7789_Fill_Color(uint16_t color)
{
    ...
    uint16_t i, j;
    ST7789_DC_Set();
    uint8_t row_buf[ST7789_WIDTH * 2];
    for (j = 0; j < ST7789_WIDTH; j++) {
        row_buf[j * 2] = color >> 8;
        row_buf[j * 2 + 1] = color & 0xFF;
    }
    for (i = 0; i < ST7789_HEIGHT; i++) {
        HAL_SPI_Transmit(&ST7789_SPI_PORT, row_buf, ST7789_WIDTH * 2, HAL_MAX_DELAY);
    }
#endif
    ST7789_UnSelect();
}
...
void ST7789_Fill(uint16_t xSta, uint16_t ySta, uint16_t xEnd, uint16_t yEnd, uint16_t color)
{
    ...
    for (i = 0; i < h; i++) {
        HAL_SPI_Transmit(&ST7789_SPI_PORT, row_buf, w * 2, HAL_MAX_DELAY);
    }
#endif
    ST7789_UnSelect();
}
...
void ST7789_WriteChar(uint16_t x, uint16_t y, char ch, FontDef font, uint16_t color, uint16_t bgcolor)
{
    ...
    for (i = 0; i < font.height; i++) {
        ...
        for (j = 0; j < font.width; j++) {
            if ((b << j) & 0x8000) {
                disp_buf[pixel_index++] = swapped_color;
            } else {
    ...
    for (i = 0; i < font.height; i++) {
        ...
        for (j = 0; j < font.width; j++) {
            if ((b << j) & 0x8000) {
                ...
                row_buf[j * 2] = bgcolor >> 8;
                row_buf[j * 2 + 1] = bgcolor & 0xFF;
            }
        }
        HAL_SPI_Transmit(&ST7789_SPI_PORT, row_buf, font.width * 2, HAL_MAX_DELAY);
    }
#endif
    ST7789_UnSelect();
}
